        ...
        if (MOVE_DATA(battler1Move).priority > 0) {
            if (MOVE_DATA(battler1Move).power > 0) {
                ...
                if (moveStatusFlags & MOVE_STATUS_TYPE_WEAKNESS_ABILITY) {
                    battler1MoveScore = battler1MoveScore * 5 / 4;
                }
    ...
    for (i = 0; i < LEARNED_MOVES_MAX; i++) {
        ...
        if (MOVE_DATA(battler2Move).priority > 0) {
            if (MOVE_DATA(battler2Move).power > 0) {
                ...
                if (moveStatusFlags & MOVE_STATUS_TYPE_WEAKNESS_ABILITY) {
                    battler2MoveScore = battler2MoveScore * 5 / 4;
                }
...
int PartyMon_ApplyTypeChart(BattleSystem *battleSys, BattleContext *battleCtx, int move, int inType, int attacker, int defender, int damage, int partyIndicator, int partySlot, u32 *moveStatusMask)
{
    ...
    if (partyIndicator == defender)
    {
        ...
        attackerItem = Battler_HeldItem(battleCtx, attacker);
        attackerType1 = BattleMon_Get(battleCtx, attacker, BATTLEMON_TYPE_1, NULL);
        attackerType2 = BattleMon_Get(battleCtx, attacker, BATTLEMON_TYPE_2, NULL);
    ...
    else
    {
        ...
        attackerType1 = Pokemon_GetValue(mon, MON_DATA_TYPE_1, NULL);
        attackerType2 = Pokemon_GetValue(mon, MON_DATA_TYPE_2, NULL);
    ...
    attackerItemEffect = BattleSystem_GetItemData(battleCtx, attackerItem, ITEM_PARAM_HOLD_EFFECT);
    attackerItemPower = BattleSystem_GetItemData(battleCtx, attackerItem, ITEM_PARAM_HOLD_EFFECT_PARAM);
    defenderItemEffect = BattleSystem_GetItemData(battleCtx, defenderItem, ITEM_PARAM_HOLD_EFFECT);
    defenderItemPower = BattleSystem_GetItemData(battleCtx, defenderItem, ITEM_PARAM_HOLD_EFFECT_PARAM);
    ...
    if ((battleCtx->battleStatusMask & SYSCTL_IGNORE_TYPE_CHECKS) == FALSE && (attackerType1 == moveType || attackerType2 == moveType)) {
    ...
    if (moveType == TYPE_GROUND
    && ((attackerAbility != ABILITY_MOLD_BREAKER && attackerAbility != ABILITY_NEUTRALIZING_GAS && defenderAbility == ABILITY_LEVITATE) || (defenderItemEffect == HOLD_EFFECT_LEVITATE_POPPED_IF_HIT))
	...
	&& ((defenderType1 == TYPE_GRASS || defenderType2 == TYPE_GRASS)
		...
		*moveStatusMask |= MOVE_STATUS_INEFFECTIVE;
		...
		else
		{
			while (sTypeMatchupMultipliers[chartEntry][0] != 0xFF) {
				...
				if (sTypeMatchupMultipliers[chartEntry][0] == moveType) {
                    if (defenderAbility == ABILITY_COLOR_CHANGE
                        && attackerAbility != ABILITY_NEUTRALIZING_GAS)
                    {
                        if (sTypeMatchupMultipliers[chartEntry][1] == moveType
                            // issue with below function where attacker battlemon will think that a partymon that is flying type
                            // will have Roost even if they don't if the current defender battlemon is flying type and has roost active.
                            // should rework BasicTypeMulApplies_PartyMon to account for defender partymon, but it's a bit of an edge case.
                    ...
                    else
                    {
                        if (sTypeMatchupMultipliers[chartEntry][1] == defenderType1
                            // same edge case as comment directly above this one
                            && BasicTypeMulApplies_PartyMon(battleSys, battleCtx, attacker, defender, chartEntry, move, partyIndicator, partySlot) == TRUE) {
                            damage = ApplyTypeMultiplier(battleCtx, attacker, sTypeMatchupMultipliers[chartEntry][2], damage, movePower, moveStatusMask);
    ...
    if ((defenderAbility == ABILITY_WONDER_GUARD && attackerAbility != ABILITY_MOLD_BREAKER && attackerAbility != ABILITY_NEUTRALIZING_GAS)
            && ((*moveStatusMask & MOVE_STATUS_SUPER_EFFECTIVE) == FALSE
                || (*moveStatusMask & MOVE_STATUS_BASIC_EFFECTIVENESS) == MOVE_STATUS_BASIC_EFFECTIVENESS)
            && movePower) {
        *moveStatusMask |= MOVE_STATUS_WONDER_GUARD;
            ...
            && (battleCtx->battleStatusMask & SYSCTL_IGNORE_IMMUNITIES) == FALSE) {
        if ((*moveStatusMask & MOVE_STATUS_SUPER_EFFECTIVE) && movePower) {
            if ((defenderAbility == ABILITY_FILTER || defenderAbility == ABILITY_SOLID_ROCK)
    ...
    if (moveType == TYPE_WATER)
	{
        if ((defenderAbility == ABILITY_WATER_ABSORB || defenderAbility == ABILITY_DRY_SKIN)
            && attackerAbility != ABILITY_MOLD_BREAKER
            && attackerAbility != ABILITY_NEUTRALIZING_GAS) {
        ...
        if (defenderAbility == ABILITY_STORM_DRAIN
            && attackerAbility != ABILITY_MOLD_BREAKER
            && attackerAbility != ABILITY_NEUTRALIZING_GAS) {
    ...
    if (moveType == TYPE_ELECTRIC)
	{
        ...
        if ((defenderAbility == ABILITY_LIGHTNING_ROD || defenderAbility == ABILITY_MOTOR_DRIVE)
            && attackerAbility != ABILITY_MOLD_BREAKER
            && attackerAbility != ABILITY_NEUTRALIZING_GAS) {
    ...
    if (moveType == TYPE_FIRE)
	{
        ...
        if ((defenderAbility == ABILITY_HEATPROOF || defenderAbility == ABILITY_THICK_FAT)
            && attackerAbility != ABILITY_MOLD_BREAKER
            && attackerAbility != ABILITY_NEUTRALIZING_GAS)
        {
            *moveStatusMask |= MOVE_STATUS_TYPE_RESIST_ABILITY;
    ...
    if (moveType == TYPE_POISON)
    {
        if (MON_HAS_TYPE(defender, TYPE_STEEL))
        {
            if (defenderAbility == ABILITY_CORROSION
                && attackerAbility != ABILITY_MOLD_BREAKER
                && attackerAbility != ABILITY_NEUTRALIZING_GAS)
            {
                *moveStatusMask |= MOVE_STATUS_TYPE_IGNORE_IMMUNITY_ABILITY;
	...
	if (defenderAbility == ABILITY_WEB_MASTER
        && attackerAbility != ABILITY_MOLD_BREAKER
        && attackerAbility != ABILITY_NEUTRALIZING_GAS) {
                    ...
                    if (BattleMon_Get(battleCtx, battler, BATTLEMON_ATTACK, NULL) >= (BattleMon_Get(battleCtx, battler, BATTLEMON_SP_ATTACK, NULL) * 2 / 3))
                    {
                        ...
                        else
                        {
                            ...
                            if (baseAtk >= baseHP
                                && baseAtk >= baseDef
                                && baseAtk >= baseSpDef
                                && Pokemon_GetValue(mon, MON_DATA_ATK_EV, NULL) > 8)
                            {
                                result = TRUE;
                            }
                        }
                ...
                if (BattleMon_Get(battleCtx, battler, BATTLEMON_ATTACK, NULL) >= (BattleMon_Get(battleCtx, battler, BATTLEMON_SP_ATTACK, NULL) * 2 / 3))
                {
                    ...
                    else
                    {
                        ...
                        if (baseSpAtk >= baseHP
                            && baseSpAtk >= baseDef
                            && baseSpAtk >= baseSpDef
                            && Pokemon_GetValue(mon, MON_DATA_SPATK_EV, NULL) > 8)
                        {
                            result = TRUE;
                        }
                    }
...
int BattleAI_CalculateStatusMoveAttackScore(BattleSystem *battleSys, BattleContext *battleCtx, int defender, int battler, int partyIndicator, int partySlot)
{
    ...
    int moveMoveEffect, moveVolatileStatus, moveStatus, moveStatFlag, moveEffect, moveFieldEffect, moveStatDropFlag;
    u8 moveType;
    ...
    for (i = 0; i < LEARNED_MOVES_MAX; i++) {
        ...
        if (MOVE_DATA(move).class == CLASS_STATUS) {
            ...
            if (((moveStatusFlags & MOVE_STATUS_IMMUNE) == FALSE)
                ...
                if ((battleCtx->fieldConditionsMask & moveFieldEffect) == FALSE)
                {
                    if (moveFieldEffect != FIELD_CONDITION_UPROAR)
                    {
                        moveScore += 25;
                    }
                ...
                if (moveEffect == BATTLE_EFFECT_TAUNT)
                {
...
int BattleAI_CalculateStatusMoveDefendScore(BattleSystem *battleSys, BattleContext *battleCtx, int defender, int battler, int partyIndicator, int partySlot)
{
    ...
    int score, moveScore;
    int defenderCurHP, defenderMaxHP;
    int moveMoveEffect, moveVolatileStatus, moveStatus, moveStatFlag, moveEffect, moveFieldEffect;
    u8 moveType;
    u8 monType1, monType2, monAbility;
    ...
    for (i = 0; i < LEARNED_MOVES_MAX; i++) 
    {
        ...
        if (MOVE_DATA(move).class == CLASS_STATUS) {
            ...
            if (((moveStatusFlags & MOVE_STATUS_IMMUNE) == FALSE)
                ...
                if (moveEffect == BATTLE_EFFECT_SWITCH_HELD_ITEMS)
                {
                    ...
                    case HOLD_EFFECT_CHOICE_ATK:
                        if (Battle_PartyMonIsPhysicalAttacker(battleSys, battleCtx, partyIndicator, partySlot) == FALSE)
                        {
                            moveScore += 25;
                        }
                        break;
                    ...
                    case HOLD_EFFECT_CHOICE_SPATK:
                        if (Battle_PartyMonIsSpecialAttacker(battleSys, battleCtx, partyIndicator, partySlot) == FALSE)
                        {
                            moveScore += 25;
                        }
                        break;
                    ...
                    case HOLD_EFFECT_PSN_USER:
                        if (Battle_AbilityDetersStatus(battleSys, battleCtx, monAbility, MON_CONDITION_POISON) == FALSE
                            && monType1 != TYPE_POISON
                            && monType2 != TYPE_POISON
                            && monType1 != TYPE_STEEL
                            && monType2 != TYPE_STEEL)
                ...
                if (compareSpeedDefenderVsMon == COMPARE_SPEED_FASTER) {
                    ...
                    case BATTLE_EFFECT_SWITCH_HELD_ITEMS:
                        ...
                        case BATTLE_EFFECT_KO_MON_THAT_DEFEATED_USER:
                            if (BattleAI_PartyMonHasPriorityMove(battleSys, battleCtx, partyIndicator, partySlot) == FALSE) {
                                moveScore += 30;
                            }
                            break;
                    }
...
{
    int i, wonderGuardCount, snowedInCount;
    int score, moveScore;
    int defenderCurHP, defenderMaxHP, defenderHPPercent;
    ...
    defenderMaxHP = BattleMon_Get(battleCtx, defender, BATTLEMON_MAX_HP, NULL);
    defenderCurHP = BattleMon_Get(battleCtx, defender, BATTLEMON_CUR_HP, NULL);
    defenderHPPercent = 100 * defenderCurHP / defenderMaxHP;
    defenderSpeedStat = BattleMon_Get(battleCtx, defender, BATTLEMON_SPEED, NULL);
    ...
    for (i = 0; i < LEARNED_MOVES_MAX; i++)
    {
        ...
        case ABILITY_WONDER_GUARD:
            if (moveStatusFlags & MOVE_STATUS_WONDER_GUARD)
            {
                wonderGuardCount++;
            }
            else
            {
                if (moveStatus & (MON_CONDITION_ANY_POISON | MON_CONDITION_BURN))
                {
                    break;
                }
                else
                {
                    if (moveMoveEffect & (MOVE_EFFECT_LEECH_SEED | MOVE_EFFECT_ABILITY_SUPPRESSED))
                    {
                        break;
                    }
                    else
                    {
                        if (moveVolatileStatus & VOLATILE_CONDITION_WONDER_GUARD)
                        {
                            break;
                        }
                        else
                        {
                            if (moveFieldEffect & (FIELD_CONDITION_SANDSTORM | FIELD_CONDITION_HAILING))
                            {
                                break;
                            }
                            else
                            {
                                if (battleCtx->fieldConditionsMask & (FIELD_CONDITION_SANDSTORM | FIELD_CONDITION_HAILING))
                                {
                                    break;
                                }
                                else
                                {
                                    if (battleCtx->sideConditionsMask[side] & SIDE_CONDITION_HAZARDS_DAMAGING)
                                    {
                                        break;
                                    }
                                    else
                                    {
                                        if (defenderAbility != ABILITY_AFTERMATH
                                            && defenderAbility != ABILITY_MOLD_BREAKER)
                                        {
                                            wonderGuardCount++;
                                        }
        ...
        case ABILITY_PRESSURE:
            if (MOVE_DATA(move).pp <= 10
                && (MOVE_DATA(move).range & RANGE_MAGIC_BOUNCE))
            {
                moveScore += 8;
            }
            break;
        ...
        if (moveFieldEffect & FIELD_CONDITION_CASTFORM)
        {
    ...
    if ((wonderGuardCount >= LEARNED_MOVES_MAX - 1
        || wonderGuardCount >= i - 1)
        && monAbility == ABILITY_WONDER_GUARD)
    {
        if (compareSpeedDefenderVsMon == COMPARE_SPEED_FASTER)
        {
            score += 200;
    ...
    case ABILITY_INTIMIDATE:
        if (Battle_BattleMonIsPhysicalAttacker(battleSys, battleCtx, defender))
        {
            if (Battle_AbilityDetersStatDrop(battleSys, battleCtx, defenderAbility, BATTLE_STAT_FLAG_ATTACK) == FALSE)
            {
                ...
                // immune to intimidate abilities list
                case ABILITY_OBLIVIOUS:
                case ABILITY_OWN_TEMPO:
                case ABILITY_INNER_FOCUS:
                case ABILITY_STEADFAST:
    ...
    case ABILITY_SHADOW_TAG:
        ...
        {
            if (defenderItemEffect != HOLD_EFFECT_SWITCH
                && defenderAbility != ABILITY_SHADOW_TAG)
            {
    ...
    case ABILITY_DOWNLOAD:
        if ((battleCtx->battleMons[defender].statusVolatile & VOLATILE_CONDITION_SUBSTITUTE) == FALSE)
        {
            if (battleCtx->battleMons[defender].defense * sStatStageBoosts[battleCtx->battleMons[defender].statBoosts[BATTLE_STAT_DEFENSE]].numerator / sStatStageBoosts[battleCtx->battleMons[defender].statBoosts[BATTLE_STAT_DEFENSE]].denominator
                < battleCtx->battleMons[battler].spDefense * sStatStageBoosts[battleCtx->battleMons[defender].statBoosts[BATTLE_STAT_SP_DEFENSE]].numerator / sStatStageBoosts[battleCtx->battleMons[defender].statBoosts[BATTLE_STAT_SP_DEFENSE]].denominator)
            {
            ...
            {
                if (Battle_PartyMonIsSpecialAttacker(battleSys, battleCtx, partyIndicator, partySlot))
                {
                    score += 25;
                }
            }
    ...
    case ABILITY_MOLD_BREAKER:
        ...
        if (PartyMon_IgnorableAbility(battleCtx, mon, defender, defenderAbility) == FALSE)
        {
            score += 25;
        }
        break;
    ...
    case ABILITY_MULTISCALE:
        if ((battleCtx->sideConditionsMask[side] & SIDE_CONDITION_STEALTH_ROCK) == FALSE)
        {
            if (((battleCtx->sideConditionsMask[side] & SIDE_CONDITION_SPIKES) == FALSE)
                || monAbility == ABILITY_LEVITATE
                || monAbility == ABILITY_MAGIC_GUARD
                || monType1 == TYPE_FLYING
                || monType2 == TYPE_FLYING)
            {
                if (((battleCtx->sideConditionsMask[side] & SIDE_CONDITION_TOXIC_SPIKES) == FALSE)
                    || monType1 == TYPE_POISON
                    || monType1 == TYPE_STEEL
                    || monType2 == TYPE_POISON
                    || monType2 == TYPE_STEEL
                    || Battle_AbilityDetersStatus(battleSys, battleCtx, monAbility, MON_CONDITION_ANY_POISON))
                {
                    score += 30;
                }
            }
    ...
    case ABILITY_CORROSION:
        if (defenderType1 == TYPE_STEEL
            || defenderType2 == TYPE_STEEL)
        {
            if ((battleCtx->battleMons[defender].status & MON_CONDITION_ANY_POISON) == FALSE)
            {
                score += 20;
            }
        }
...
int BattleAI_CalculateDamagingMoveAttackScore(BattleSystem *battleSys, BattleContext *battleCtx, int defender, int battler, int partyIndicator, int partySlot)
{
    int i;
    int score, moveScore;
    int defenderCurHP, defenderMaxHP;
        ...
        if (MOVE_DATA(move).power > 0) {
            ...
            if (moveEnemyStatDropStatFlag != BATTLE_STAT_FLAG_NONE)
            {
                if (defenderAbility != ABILITY_DEFIANT
                    && defenderAbility != ABILITY_COMPETITIVE)
                {
                    switch (moveEnemyStatDropStatFlag)
                    {
                    default:
                        break;

                    case BATTLE_STAT_FLAG_SPEED:
                        if (compareSpeedDefenderVsMon != COMPARE_SPEED_SLOWER)
                        {
                            moveScore += 15;
                        }
                    ...
                    case BATTLE_STAT_FLAG_SP_ATTACK:
                        if (Battle_BattleMonIsSpecialAttacker(battleSys, battleCtx, defender))
                        {
                            moveScore += 10;
                        }
                        break;

                    case BATTLE_STAT_FLAG_ACCURACY:
                        moveScore += 8;
                        break;
                    }
                }
            ...
            if (MOVE_DATA(move).priority > 0)
            {
                ...
                if (defenderCurHP < moveScore)
                {
                    if (BattleAI_BattleMonHasPriorityMove(battleSys, battleCtx, defender) == FALSE)
                    {
                        // defender doesn't have a priority move, defender is faster or tied for speed, mon has KO on defender
                        if (compareSpeedDefenderVsMon != COMPARE_SPEED_SLOWER)
                        {
                            moveScore += 100;
                        }
...
BOOL Battle_PartyMonAbilityDetersContactMove(BattleSystem *battleSys, BattleContext *battleCtx, int defender, int attacker, int partyIndicator, int partySlot)
{
    ...
    if (partyIndicator == defender)
    {
        ...
        defenderGender = Pokemon_GetValue(mon, MON_DATA_GENDER, NULL);
        defenderType1 = Pokemon_GetValue(mon, MON_DATA_TYPE_1, NULL);
        defenderType2 = Pokemon_GetValue(mon, MON_DATA_TYPE_2, NULL);
        defenderStatusMask = Pokemon_GetValue(mon, MON_DATA_STATUS_CONDITION, NULL);
        defenderAbility = Pokemon_GetValue(mon, MON_DATA_ABILITY, NULL);

        defenderHeldItem = Pokemon_GetValue(mon, MON_DATA_HELD_ITEM, NULL);
    ...
    else
    {
        ...
        attackerGender = Pokemon_GetValue(mon, MON_DATA_GENDER, NULL);
        attackerType1 = Pokemon_GetValue(mon, MON_DATA_TYPE_1, NULL);
        attackerType2 = Pokemon_GetValue(mon, MON_DATA_TYPE_2, NULL);
        attackerStatusMask = Pokemon_GetValue(mon, MON_DATA_STATUS_CONDITION, NULL);
        attackerAbility = Pokemon_GetValue(mon, MON_DATA_ABILITY, NULL);
