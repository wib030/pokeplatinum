...
static u8 TrainerAI_MainSingles(BattleSystem *battleSys, BattleContext *battleCtx)
{
    ...
    u8 predictAction = AI_ENEMY_ATTACK_1;
    u16 move;
...
static void AICmd_IfAbilityInPlay(BattleSystem* battleSys, BattleContext* battleCtx)
{
    ...
    int ability = AIScript_Read(battleCtx);
    int jump = AIScript_Read(battleCtx);
...
static void AICmd_IfBattlerHasBounceableMove(BattleSystem* battleSys, BattleContext* battleCtx)
{
    ...
    if (bounceableMoves)
    {
        // 3 or 4 bounceable moves
        if (bounceableMoves > 2)
        {
            hasBounceableMove = TRUE;
        }
        else
        {
            // 2 bounceable moves
            if (bounceableMoves > 1)
            {
                if (BattleSystem_RandNext(battleSys) % 8)
                {
                    hasBounceableMove = TRUE;
                }
            }
            ...
            {
                // Special case for battler 1's first turn out
                if (battleCtx->battleMons[battler1].moveEffectsData.fakeOutTurnNumber < battleCtx->totalTurns)
                {
                    if (BattleSystem_RandNext(battleSys) % 8)
                    {
                        hasBounceableMove = TRUE;
